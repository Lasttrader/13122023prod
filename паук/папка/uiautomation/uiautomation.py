...
def WalkControl(control: Control, includeTop: bool = False, maxDepth: int = 0xFFFFFFFF):
    ...
    while depth >= 0:
        ...
        if lastControl:
            ...
            depth -= 1
def LogControlNewdepth(control: Control, depth: int = 0, showAllName: bool = True, showPid: bool = False) -> int:
    indent = int(depth )
    return indent

def LogControlNewSmal(control: Control, depth: int = 0, showAllName: bool = True, showPid: bool = False) -> str:
    row=""       
    row = row + f'ClassName: {control.ClassName}'   
    row = row + f'| Name: {control.Name}'
    return row
def LogControlNew(control: Control, depth: int = 0, showAllName: bool = True, showPid: bool = False) -> str:
    row=""
    indent = depth #' ' * depth * 4
  
    row = row + f'ControlType: {control.ControlTypeName}'
    row = row + f'| ClassName: {control.ClassName}'
    row = row + f'| AutomationId: {control.AutomationId}'
    row = row + f'| BoundingRectangle: {control.BoundingRectangle}'
    row = row + f'| Name: {control.Name}'
    row = row + f'| Handle: {control.NativeWindowHandle}'
    row = row + f'| Depth: {depth}'

    if showPid:
        row = row + f'| ProcessId: {control.ProcessId}'
        row = row + f'| BoundingRectangle: {control.BoundingRectangle}'
    ...
    for pt, name in supportedPatterns:
        if isinstance(pt, ValuePattern):
            row = row + f'| ValuePattern.Value: {repr(pt.Value)}'
        elif isinstance(pt, RangeValuePattern):
            row = row + f'| RangeValuePattern.Value: {pt.Value}'
        elif isinstance(pt, TogglePattern):
            row = row + f'| TogglePattern.ToggleState: ToggleState.{_GetDictKeyName(ToggleState.__dict__, pt.ToggleState)}'
        elif isinstance(pt, SelectionItemPattern):
            row = row + f'| SelectionItemPattern.IsSelected: {pt.IsSelected}'
        elif isinstance(pt, ExpandCollapsePattern):
            row = row + f'| ExpandCollapsePattern.ExpandCollapseState: {_GetDictKeyName(ExpandCollapseState.__dict__, pt.ExpandCollapseState)}'
        elif isinstance(pt, ScrollPattern):
            row = row + f'| ScrollPattern.HorizontalScrollPercent: {pt.HorizontalScrollPercent}'
            row = row + f'| ScrollPattern.VerticalScrollPercent: {pt.VerticalScrollPercent}'
        elif isinstance(pt, GridPattern):
            row = row + f'| GridPattern.RowCount: {pt.RowCount}'
            row = row + f'| GridPattern.ColumnCount: {pt.ColumnCount}'
        elif isinstance(pt, GridItemPattern):
            row = row + f'| GridItemPattern.Row: {pt.Row}'
            row = row + f'| GridItemPattern.Column: {pt.Column}'
        elif isinstance(pt, TextPattern):
            try:
                row = row + f'| TextPattern.Text: {repr(pt.DocumentRange.GetText(30))}'
            except comtypes.COMError as ex:
                pass
    for pt, name in supportedPatterns:
...
def EnumAndLogControlAncestors(control: Control, showAllName: bool = True, showPid: bool = False) -> None:
    ...
    for i, control in enumerate(lists):
        print(LogControlNew(control, i, showAllName, showPid))
        #print(control, i, showAllName, showPid)
